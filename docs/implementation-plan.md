...
*Revision 6 - Task 3D.1 (Buzzer) complete: unit defined, swarm-spawn logic wired in, confirmed loading clean in-game with no additional sprite needed for flying units.*
...
│   │   └── buzzer.hjson         # Buzzer unit definition (Task 3D.1, done)
...
│   ├── spawn-trigger.js          # pure spawn-decision logic, keyed by (x,y,unitType) as of Task 3D.1
│   ├── spawn-utils.js            # NEW: shared findNearestOpenTile (extracted during Task 3D.1)
│   ├── spawn-hook.js             # engine wiring: spawns real Skitters
│   └── buzzer-hook.js            # NEW: engine wiring: spawns Buzzer swarms (2-3 units)
├── tests/
│   ├── noise-tracker.test.js
│   ├── noise-hook.test.js
│   ├── spawn-trigger.test.js
│   ├── spawn-hook.test.js
│   └── buzzer-hook.test.js       # NEW
├── sprites/units/
│   ├── skitter.png / skitter-cell.png    # placeholder sprites
│   └── buzzer.png / buzzer-cell.png      # placeholder sprites - confirmed sufficient, no shadow sprite needed for flying units
└── docs/
...
**Planned additions for the rest of Phase 3B (not yet built):**
```
scripts/
...
- The wave mechanism (Phase 3B) will spawn *multiple* units at once, per wave, scaled with noise/duration - this multiplies the pathfinding-cost concern already flagged for single spawns. Buzzer's swarm spawning (Task 3D.1) is a first, smaller-scale test of this same concern (2-3 units per trigger instead of 1) - worth watching for any TPS impact even before the full wave mechanism lands.
...
**Task 2.2 - Spawn trigger logic** - DONE (`scripts/spawn-trigger.js` + `spawn-hook.js`, real single-unit spawning works, spawn-on-building bug fixed). **Refactored during Task 3D.1** to key state by `(x, y, unitType)` instead of just `(x, y)` - see Section 6.

*(See rev 3 for full detail on Phase 2 - unchanged otherwise. Two real bugs were found and fixed during this phase; both documented in Section 6.)*

### Phase 3: Enemy Unit & AI (Skitter) - **Task 3.1 done, Task 3.2 postponed, Task 3.3 implemented but dormant**
...
- "Random empty area close to the faction's core that makes noise" - different from the single-spawn mechanism's "nearest open tile to the specific source" (now `spawn-utils.js`'s `findNearestOpenTile`)
...
- Ties 3B.1-3B.4 together in a tick-batched hook, following the established `noise-hook.js`/`spawn-hook.js`/`buzzer-hook.js` pattern (confirmed API conventions from Section 6 apply here too - `Events.run`, no `.js` in `require()`, pause-checking, etc.)
...
- Sub-steps mirror Task 3.1/3D.1's now-proven process: define `.hjson`, generate placeholder sprite(s) (body + cell only - confirmed sufficient even for flying units, very likely fine for ground too), verify clean in-game load, fix any warnings/errors the same way `skitter.hjson`'s `range` field warning was fixed
...
**Task 3D.1 - Buzzer (Tier 1 flying) - DONE**
- `type: flying`, defined in `content/units/buzzer.hjson`. Loaded clean in-game with zero warnings or errors.
- **CONFIRMED via live testing: flying units need no additional sprite beyond body + cell overlay** - no shadow sprite or other flying-specific asset was required. This should apply to Shrieker and Screecher too (also flying).
- Weapon: Acid Spit - short-range, fast fire rate, low damage. The splash/puddle damage-over-time effect described in `mod-units.md` is **not implemented** - no confirmed bullet type supports it yet; left as a plain single-target `BasicBulletType` bullet for now.
- Spawn side: `scripts/buzzer-hook.js` spawns a swarm of 2-3 on the same "any active mining noise" trigger as Skitter, via the single-drill spawn mechanism (Task 2.2), not the wave mechanism - independent cooldown/cap from Skitter, thanks to the `spawn-trigger.js` refactor (see Section 6).
- Still uses the shared `BUG_TEAM` (`Vars.state.rules.waveTeam`, i.e. `Team.crux`), same as Skitter, per Task 3E.2's reversion.
- 53 tests passing across all suites as of this task, including a real tile-collision bug caught and fixed by the tests themselves before ever reaching the live game (see Section 6).
...
- This is a genuine architectural extension of Phase 2's spawn logic, not a drop-in addition - plan for it to touch `spawn-trigger.js` directly rather than living entirely in a new file. Note: `spawn-trigger.js` already gained one real architectural extension during Task 3D.1 (per-unit-type keying) - this task will be its second.
- *Dependencies:* Task 3B (wave mechanism), all six new unit definitions existing (3C.1-3C.2, 3D.1-3D.3, of which 3D.1 is done) so the ladder has real units to select from
...
- **PART 2: REAL BLOCKER FOUND, unresolved.** Attempted to apply this by switching Skitter's spawn team from `Vars.state.rules.waveTeam` (`Team.crux`) to a dedicated `Team.green`. Result: spawned Skitters were destroyed **immediately, at full health** (confirmed via a live `UnitDestroyEvent` listener: `health=35 maxHealth=35`, not combat damage). `Team.crux` never showed this behavior.
  ...
  - **Action taken:** reverted to `Vars.state.rules.waveTeam` (proven stable) rather than ship a broken faction change. Both Skitter and now Buzzer use this same team. See `scripts/spawn-hook.js`'s `BUG_TEAM` constant and its surrounding comment for the full record.
...
- **Not every engine question resolves cleanly via live testing.** Task 3E.2 remains the clearest example: live testing produced a clear, reproducible negative result (unit cap ruled out) without fully explaining the underlying mechanism. The response was to revert to a known-good state and document the open question honestly, rather than ship a guessed "fix" for a poorly-understood problem.
- **New (rev 6): shared code extraction is paying off.** Extracting `findNearestOpenTile` into `spawn-utils.js` during Task 3D.1, and refactoring `spawn-trigger.js` to key by unit type, both existed specifically because Buzzer needed to reuse Skitter's patterns without duplicating or corrupting them. Worth continuing this instinct as more units are added - Task 3C.1 (Gnawer) explicitly notes it should reuse the now-proven unit-definition process rather than start fresh.
...
**Resolved in rev 6:**
- ~~Do flying units need extra sprites (shadow, etc.)?~~ → **No.** Confirmed via Buzzer's clean live load: body + cell overlay is sufficient, same as ground units.
...
- What actually causes a unit spawned on a coreless, non-`crux` team to be destroyed immediately (Task 3E.2)?

**Immediate next steps:**
1. Task 3E.2 remains a genuine open research problem, not a quick win - don't re-attempt with another arbitrary team constant without a stronger hypothesis first
2. Reasonable next picks: **Task 3C.1/Gnawer** (a ground unit, now following the proven Task 3.1/3D.1 pattern) or **Task 3B.1** (faction noise aggregation - can be built using `Team.crux`/`waveTeam` for now, and swapped later if/when Task 3E.2 resolves)
3. Task 5.1's formal TPS protocol should be scheduled once Phase 3B-i (wave mechanism) lands, given the raised performance stakes noted in Section 4 - though an informal glance during Buzzer swarm spawns wouldn't hurt sooner
...
- **`Groups.build` has no indexed access** - no `.size` property, no `.get(i)`. Only `.each(Cons)`. Confirmed via direct console probing after a silent 5-minute-long bug.
...
- **`Trigger.update` fires even while the game is paused.** `Vars.state.isPaused()` must be checked explicitly in every tick-driven hook.
- **Mod content names are namespaced as `<modname>-<contentname>`**, not the plain filename stem. Confirmed for Skitter (`skitter-mod-skitter`); Buzzer's lookup (`skitter-mod-buzzer`) follows the same pattern and loaded without error.
...
- **Unit `type` (Hjson) must be one of exactly:** `flying/mech/legs/naval/payload/missile/tether/crawl`. Confirmed via an in-game `IllegalArgument` error listing the full valid set.
  ...
  - **`flying`** (Buzzer): confirmed valid and working - loaded clean in-game. **Shrieker and Screecher should use this too.**
- **Flying units need no additional sprite beyond body + cell overlay.** Confirmed via Buzzer's clean live load - no shadow sprite or other flying-specific asset required.
...
- **`Vars.world.solid(x, y)`** correctly identifies occupied/blocked tiles, confirmed against `World.java` source. Note: this checks TERRAIN/BUILDING solidity only, NOT whether another unit already occupies a tile - discovered as a real bug during Task 3D.1's swarm spawning (multiple Buzzers all finding the same "open" tile, since a previously-spawned unit doesn't register as solid). Fixed with an exclusion-set parameter on `findNearestOpenTile` (`spawn-utils.js`) that swarm-spawning callers populate themselves as they go.
...
- **`Sounds.spawn` does NOT exist.** Confirmed via a live `InternalError`. A guessed field name from a wiki page that mixed sound and visual-effect (Fx) names without distinguishing them - removed rather than guessed again.
...
- **`Teams` (the manager class, `Vars.state.teams`) has no `areEnemies()` method** - confirmed via a live `EvaluatorException` when guessed. The real hostility-adjacent methods, found via reflection, are `closestEnemyCore(float, float, Team)`, `anyEnemyCoresWithin(Team, float, float, float)`, and `eachEnemyCore(Team, Cons)`.
- **A unit spawned on a team with zero cores can be destroyed immediately, at full health, for a reason that is NOT unit cap.** Confirmed: Skitters spawned on `Team.green` died the same tick they spawned. `Team.crux` never shows this behavior, and `unitCap` was confirmed identical (`0`) for both teams, ruling that out as the cause. **The actual mechanism is unconfirmed** - reverted to `Team.crux`/`waveTeam`. See Task 3E.2 for full context.
